...
- **Background glows (WelcomePage hero band):** low-opacity (8-16%) blurred blobs with a slow, independent drift + flicker (`hero-glow-a/b/c` classes in `index.css`, ~9-13s position drift + ~4-5.5s opacity flicker, staggered delays so blobs don't move in lockstep) - "distinct but classic" ambient motion, replacing the earlier fully-static pair and the "Vibrant Craft" era's `animate-pulse` (which read as a generic AI-landing-page tell; a single uniform pulse reads as a tell in a different way than this staggered multi-blob drift does). **Theme-specific treatment:** in light theme, two copper-only blobs drift calmly; in dark theme, the hero background itself is pure near-black (`#050403` vs light theme's `#15100d`) and four blobs alternate between the copper accent and blue (`#3b82f6`/`#2563eb`) for a "fire" effect - warm and cool tones drifting/flickering independently against black, per explicit user request ("in dark mode, make it black with fire animation that is blueish and orange").
