...
- **Hero glass panel:** the WelcomePage hero's content (logo, headline, CTAs) sits inside a frosted-glass card (`bg-white/[0.06]`, `backdrop-blur-xl`, `border-white/10`, `rounded-3xl`, `shadow-2xl`) rather than directly on the flat dark band, so the animated corner glows diffuse through the translucent backdrop behind the text - per explicit user request ("make the hero band glassy").
