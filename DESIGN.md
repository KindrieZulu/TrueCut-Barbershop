...
- **Site frame:** a literal 6px solid black border wraps the whole page (all four edges, scrolls with content so it reads as a frame around the entire site rather than a fixed viewport overlay) - per explicit user request referencing the same car-marketplace site's black demarcation border. Always black regardless of theme, since it's a fixed design/frame element, not a themed surface. Implemented as an inline `style` attribute on `<body>` in `index.html`, not as a CSS rule in `index.css` - this project's build pipeline has been observed to silently drop hand-authored `border`/`box-sizing` declarations added to a plain `body {}` selector in that file (the same class of bug already documented above for `html`/`:root` rules), so the fix lives inline, matching the workaround `ThemeContext.tsx` already uses for the same underlying issue.
- **WelcomePage hero band:** the hero (Section 1 - logo, clock, eyebrow, headline, feature pills, CTA buttons) sits in its own full-bleed band with a fixed dark background (`#15100d`, a literal hex, not a `--tc-*` token) regardless of the site's light/dark theme toggle. Per explicit user request: in light theme, the hero previously shared the same light beige background as the navbar above it and the role-selection section below it, reading as "washed out" with no visual separation - matching the car-marketplace reference site's light-navbar/dark-hero contrast fixed this. Every color inside this band (text, icons, pill backgrounds, gradients) is hardcoded to a fixed value rather than a theme token, since it must stay legible against this permanently-dark backdrop no matter which theme is active - notably using `text-gray-50`/`text-gray-200`/`text-gray-300` (Tailwind literals not remapped by `tailwind.config.js`) rather than the app's usual `text-white`/`text-gray-400`, both of which are theme-reactive (or, for `text-white` specifically, actively repainted to the theme's dark text color in light mode - see the note above) and would render illegibly dark against this permanently-dark band once the site is switched to light theme.
